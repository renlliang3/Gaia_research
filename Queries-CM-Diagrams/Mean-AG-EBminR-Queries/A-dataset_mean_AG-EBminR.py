...
for i in range(len(results)):
	if results['bp_rp'][i]-extra_data['EBminR'][i] >= 0.0 and results['bp_rp'][i]-extra_data['EBminR'][i] <= 0.33 and (results['phot_g_mean_mag'][i] + 5 * np.log10(results['parallax'][i]) - 10 - extra_data['AG'][i]) <= (3*(results['bp_rp'][i]-extra_data['EBminR'][i]) + 2.1):
		if k==2:
			A_sources = np.append(results[int(first_indices[0])],results[int(first_indices[1])])
			k+=1
		elif k>2:
			A_sources = np.append(A_sources, results[i])
		else:
			first_indices = np.append(first_indices,i)
			k+=1

t_A = Table(A_sources)
t_A.write('A_corrected_mean_AG_EBminR.fits', format='fits')
