...
rel_path3 = "Data/OB-Katz.fits"
abs_file_path3 = os.path.join(script_dir, rel_path3)

readresults = Table.read(abs_file_path3,format='fits')
...
for  i in range(N):
    ...
    y = int(round(float(ygal[i])/binSize))+w
    z = int(round(float(zgal[i])/binSize))+z_height
    if x >= 0 and x < 2*w and y >= 0 and y < 2*w and z >= 0 and z < 2*z_height and bmask[x][y][z] == True:
        source_id_in.append(results['source_id'][i])
np.savetxt('OB-Katz-contour-0.1.dat', np.c_[source_id_in], fmt = '%21i')
...
for  i in range(N):
    ...
    if x >= 0 and x < 2*w and y >= 0 and y < 2*w and z >= 0 and z < 2*z_height and bmask[x][y][z] == True:
        source_id_in.append(results['source_id'][i])
np.savetxt('OB-Katz-contour-0.2.dat', np.c_[source_id_in], fmt = '%21i')
...
for  i in range(N):
    ...
    if x >= 0 and x < 2*w and y >= 0 and y < 2*w and z >= 0 and z < 2*z_height and bmask[x][y][z] == True:
        source_id_in.append(results['source_id'][i])
np.savetxt('OB-Katz-contour-0.3.dat', np.c_[source_id_in], fmt = '%21i')
