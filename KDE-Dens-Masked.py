...
readresults2 = Table.read('Velacontour4.5e10-7.dat',format='ascii')
...
mlab.contour3d(X, Y, Z, dens, contours=[2.1*10**-7,2.5*10**-7,3.5*10**-7,4.5*10**-7], opacity=0.3, vmin=np.min(dens), vmax=np.max(dens), colormap = 'Blues', figure = figure)
...
mlab.points3d(x_OD, y_OD, z_OD, np.full(len(x_OD),1), scale_factor = 1, color=(0,0,0), figure = figure)
...
for i in range(len(x)):
        ...
        x_voxel = int(round(float((x[i]+50)/binsize))) + w
        y_voxel = int(round(float((y[i]+350)/binsize))) + w
...
np.savetxt('Velacontour4.5e10-7.dat', new_arr, header='source_id xg yg zg vx vy vz', fmt = '%2li %1.8f %1.8f %1.8f %1.8f %1.8f %1.8f')
"""
