...
with open('DR2_Radial_Velocity.csv') as csvdata:
	...
	for row in readcsvdata:
		if row[9] and row[11] and row[12] and row[50] and row[55] and row[60] and row[63] and row[66] and row[81] and row[84]: #Check if the wanted parameters have values
			if (float(row[9]) > 1000/1000) and (float(row[11]) > 5.0) and ((float(row[50])+5*np.log10(float(row[9]))-10-float(row[81])) < 2) and (float(row[63])-float(row[84]) < 0): #d<1000pc and parallax_over_error>5.0 and M_G (intrinsic) < 2 and (G_BP-G_RP)(Intrinsic) < 0
				readresults1000.append(row)
...
readresults1000.insert(0,headerinfo)

with open('Reduced_DR2_Radial_Velocity_OB-cut', 'w') as myfile:
    wr = csv.writer(myfile, quoting=csv.QUOTE_NONE)
    wr.writerows(readresults1000)
