...
readresults = Table.read('First_Eight_fits.fits',format='fits')
#Too much data, read in row by row with the list appending and then use that data.
...
for i in range(len(distances)):
	if (distances[i] < 650) & (results['parallax_over_error'] >= 5.0):
		list650.append(i)
	if (distances[i] < 1000) & (results['parallax_over_error'] >= 5.0):
		list1000.append(i)
...
distances = np.array([results['distances'][i] for i in list])

pmra_cosdec = pmra*np.cos(dec*np.pi/180)
                           
coordinates_ICRS = asc.SkyCoord(ra=ra*u.degree, dec=dec*u.degree, distance=distances*u.pc, pm_ra_cosdec=pmra_cosdec*u.mas/u.yr, pm_dec=pmdec*u.mas/u.yr, radial_velocity=radial_velocity*u.km/u.s, frame='icrs', obstime='J2015.5')
